    ...
    if "aww" in (message.text).lower():
        ...
        try:
            ...
            if "can't remove chat owner" in err.result_json.get("description"):
                bot.reply_to(
                    message, "Oops, Chat Owner can use these forbidden words!")
            ...
            if "user is an administrator of the chat" in err.result_json.get(
                ...
                bot.reply_to(
                    message, "Chat Admins can also use these forbidden words!")
            # checks if message is sent directly to bot
                ...
                bot.reply_to(
                    message, "Sorry, This doesn't work in private chats!")
            # log error to sentry
            else:
                logger.error(err)
                raise
